...
// Fast path: non-causal, Dh is a multiple of 16 (Dh = k * 16), M=4 queries at a time.
// Qh: [H,T,Dh], KhT: [H,Dh,T], Vh: [H,T,Dh], Ctx: [T,H*Dh]
// Works like the Dh=16 kernel but loops over Dh/16 segments.
...
{
  const int S = Dh / 16;                    // number of 16-wide segments
  const __m512 vscale = _mm512_set1_ps(1.0f / std::sqrt(16.0f));
  const int BK = 128;                       // key block size (try 96/128/160 if you want to sweep)

  for (int t0 = 0; t0 < T; t0 += 4) {
    const int R = std::min(4, T - t0);

    // Pointers to 4 ctx rows (each length H*Dh)
    float* ctx0 = Ctx.data() + (size_t)(t0+0)*H*Dh;
    float* ctx1 = (R>1) ? (Ctx.data() + (size_t)(t0+1)*H*Dh) : ctx0;
    float* ctx2 = (R>2) ? (Ctx.data() + (size_t)(t0+2)*H*Dh) : ctx0;
    float* ctx3 = (R>3) ? (Ctx.data() + (size_t)(t0+3)*H*Dh) : ctx0;
    ...
    for (int h = 0; h < H; ++h) {
      ...
      // Accumulators per query × per 16-wide segment
      std::vector<__m512> acc0(S, _mm512_setzero_ps());
      std::vector<__m512> acc1(S, _mm512_setzero_ps());
      std::vector<__m512> acc2(S, _mm512_setzero_ps());
      std::vector<__m512> acc3(S, _mm512_setzero_ps());

      for (int j0 = 0; j0 < T; j0 += BK) {
        const int take = std::min(BK, T - j0);

        // Process the block in halves of 64 keys to limit register pressure.
        for (int half = 0; half < 2 && half*64 < take; ++half) {
          const int base   = j0 + half*64;
          const int left   = take - half*64;
          const int groups = std::min(4, (left + 15) / 16); // number of 16-key groups in this half

          // s[g][q] logits for 4 queries × up to 4 groups
          __m512 s00=_mm512_setzero_ps(), s01=_mm512_setzero_ps(),
                 s02=_mm512_setzero_ps(), s03=_mm512_setzero_ps(); // q0
          __m512 s10=_mm512_setzero_ps(), s11=_mm512_setzero_ps(),
                 s12=_mm512_setzero_ps(), s13=_mm512_setzero_ps(); // q1
          __m512 s20=_mm512_setzero_ps(), s21=_mm512_setzero_ps(),
                 s22=_mm512_setzero_ps(), s23=_mm512_setzero_ps(); // q2
          __m512 s30=_mm512_setzero_ps(), s31=_mm512_setzero_ps(),
                 s32=_mm512_setzero_ps(), s33=_mm512_setzero_ps(); // q3

          // Accumulate logits over Dh in 16-wide segments
          for (int seg = 0; seg < S; ++seg) {
            const int dbase = seg * 16;
            // For each lane d0 within the 16-wide segment: broadcast q[d] and FMA with K^T[d, keys]
#pragma unroll(16)
            for (int d0 = 0; d0 < 16; ++d0) {
              const int d = dbase + d0;
              const float* kt = KhT.data() + ((size_t)h*Dh + d)*T + base;
              __m512 kd0 = (groups>=1) ? _mm512_loadu_ps(kt +  0) : _mm512_setzero_ps();
              __m512 kd1 = (groups>=2) ? _mm512_loadu_ps(kt + 16) : _mm512_setzero_ps();
              __m512 kd2 = (groups>=3) ? _mm512_loadu_ps(kt + 32) : _mm512_setzero_ps();
              __m512 kd3 = (groups>=4) ? _mm512_loadu_ps(kt + 48) : _mm512_setzero_ps();
          ...
          // scale logits
          s00=_mm512_mul_ps(s00, vscale); if (groups>=2) s01=_mm512_mul_ps(s01, vscale);
          if (groups>=3) s02=_mm512_mul_ps(s02, vscale); if (groups>=4) s03=_mm512_mul_ps(s03, vscale);
          if (R>1){ s10=_mm512_mul_ps(s10, vscale); if (groups>=2) s11=_mm512_mul_ps(s11, vscale);
                    if (groups>=3) s12=_mm512_mul_ps(s12, vscale); if (groups>=4) s13=_mm512_mul_ps(s13, vscale); }
          if (R>2){ s20=_mm512_mul_ps(s20, vscale); if (groups>=2) s21=_mm512_mul_ps(s21, vscale);
                    if (groups>=3) s22=_mm512_mul_ps(s22, vscale); if (groups>=4) s23=_mm512_mul_ps(s23, vscale); }
          if (R>3){ s30=_mm512_mul_ps(s30, vscale); if (groups>=2) s31=_mm512_mul_ps(s31, vscale);
                    if (groups>=3) s32=_mm512_mul_ps(s32, vscale); if (groups>=4) s33=_mm512_mul_ps(s33, vscale); }

          auto blkmax4 = [&](const __m512 a0,const __m512 a1,const __m512 a2,const __m512 a3,int g)->float{
            float bm = attn::hmax_ps(a0);
            if (g>=2) bm = std::max(bm, attn::hmax_ps(a1));
            if (g>=3) bm = std::max(bm, attn::hmax_ps(a2));
            if (g>=4) bm = std::max(bm, attn::hmax_ps(a3));
            return bm;
          };
          const float bm0 = blkmax4(s00,s01,s02,s03,groups);
          const float bm1 = (R>1) ? blkmax4(s10,s11,s12,s13,groups) : -INFINITY;
          const float bm2 = (R>2) ? blkmax4(s20,s21,s22,s23,groups) : -INFINITY;
          const float bm3 = (R>3) ? blkmax4(s30,s31,s32,s33,groups) : -INFINITY;
          ...
          float l_blk0 = attn::hsum_ps(w00); if (groups>=2) l_blk0 += attn::hsum_ps(w01);
          if (groups>=3) l_blk0 += attn::hsum_ps(w02); if (groups>=4) l_blk0 += attn::hsum_ps(w03);

          float l_blk1 = 0.f, l_blk2 = 0.f, l_blk3 = 0.f;
          if (R>1){ l_blk1  = attn::hsum_ps(w10); if (groups>=2) l_blk1 += attn::hsum_ps(w11);
                    if (groups>=3) l_blk1 += attn::hsum_ps(w12); if (groups>=4) l_blk1 += attn::hsum_ps(w13); }
          if (R>2){ l_blk2  = attn::hsum_ps(w20); if (groups>=2) l_blk2 += attn::hsum_ps(w21);
                    if (groups>=3) l_blk2 += attn::hsum_ps(w22); if (groups>=4) l_blk2 += attn::hsum_ps(w23); }
          if (R>3){ l_blk3  = attn::hsum_ps(w30); if (groups>=2) l_blk3 += attn::hsum_ps(w31);
                    if (groups>=3) l_blk3 += attn::hsum_ps(w32); if (groups>=4) l_blk3 += attn::hsum_ps(w33); }

          // Store weights into arrays so we can reuse V rows across 4 queries
          ...
          // Accumulate V: load each V row once, FMA into 4 queries × S segments
          auto fma_group = [&](int gidx, int count){
          ...
          l0 = l0 * a0 + l_blk0; m0 = m0_new;
          if (R>1){ l1 = l1 * a1 + l_blk1; m1 = m1_new; }
          if (R>2){ l2 = l2 * a2 + l_blk2; m2 = m2_new; }
          if (R>3){ l3 = l3 * a3 + l_blk3; m3 = m3_new; }
        } // half
      } // blocks

      // Normalize and write to ctx rows
