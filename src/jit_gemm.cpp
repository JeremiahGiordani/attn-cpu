// jit_gemm.cpp — AVX-512 SGEMM with Xbyak JIT micro-kernel (near jumps + callee-saved)
// - Global α-fused packing (A) and packing (B)
// - Column-parallel B tiles; K split into L1-sized Kc panels
// - JIT 8×48 micro-kernel, k-unroll=2, beta==0, aligned B loads, NT stores
// - Portable fallback for beta!=0 and tails
//
...
static constexpr int MR    = 8;     // micro-kernel rows
static constexpr int NR    = 48;    // micro-kernel cols (3× zmm)
static constexpr int KC_L1 = 768;   // MR×KC_L1×4 ≈ 24 KB (A slice fits L1)
...
// ===== JIT micro-kernel (beta==0, aligned) =====
//
// void ker(const float* Ap, const float* Bp,
//          float* C, int ldc, int K, int stream_flag);
//
// 8×48, k-unroll=2, K split into Kc panels. No spills (24 acc + 6 B + 2 temps).
struct Jit8x48_Beta0_Align : public Xbyak::CodeGenerator {
  ...
  Jit8x48_Beta0_Align() {
    ...
    const Reg64 kbase = r10;     // current K offset
    const Reg64 a_ptr = r11;
    const Reg64 b_ptr = r12;
    const Reg64 c_row = r13;
    const Reg64 ldc_bytes = r14;
    const Reg64 tmp = r15;       // general temp
    const Reg64 kcnt = rax;      // counter within panel
    const Reg64 Kc = rbx;        // panel size

    // Prologue (save callee-saved we use)
    push(rbp); mov(rbp, rsp);
    push(rbx); push(r12); push(r13); push(r14); push(r15);

    // Zero accumulators: acc0[r]=zmm0..7, acc1[r]=zmm8..15, acc2[r]=zmm16..23
    ...
    L(L_outer);
      ...
      // Kc = min(KC_L1, remaining)
      mov(Kc, (uint64_t)KC_L1);
      cmp(kcnt, KC_L1);
      ...
      // ---- inner: k-unroll=2 across this panel ----
      mov(kcnt, Kc);                // kcnt = Kc
      L(L_panel_loop);
        cmp(kcnt, 2);
    ...
    // ---- stores: beta==0; choose NT vs storeu once, then unrolled rows ----
    Label L_storeu_all, L_done_all;

    // if (!stream) goto storeu_all;
    test(stream, stream);
...
#pragma omp parallel for schedule(static)
  ...
  // Column-parallel sweep (each thread owns B tile(s))
  if (beta == 0.0f) {
...
#pragma omp parallel for schedule(static)
    for (int tn = 0; tn < NT; ++tn) {
      ...
      for (int tm = 0; tm < MT; ++tm) {
        ...
        if (mr_eff == MR && nr_eff == NR) {
          // stream if all three 16-float chunks are 64B aligned
    ...
    // Portable β!=0 path (compute then fold β and store once)
#pragma omp parallel for schedule(static)
    for (int tn = 0; tn < NT; ++tn) {
      int j0 = tn * NR;
      const float* Bp = B_pack + (size_t)tn * K * NR;
